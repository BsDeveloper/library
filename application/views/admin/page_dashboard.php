                ...
                <script>
                    init.push(function () {
                        var uploads_data = [
                            ...
                            <?php $jumlah_unique = $this->db->query("select count(jml) as total from (select count(*) as jml from log_visitor where date(create_date) = date('$row->create_date') group by ip_address) t")->row()->total; ?>

                            { day: '<?php echo date("Y-m-d",strtotime($row->create_date))?>', v: <?php echo $row->total?>, u: <?php echo $jumlah_unique?> },
