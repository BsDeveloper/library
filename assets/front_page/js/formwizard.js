...
formtowizard.prototype={
  	...
  	//doload Boolean checks to see whether to load next section (true if bypasshooks param is true or onpagechangestart() event handler doesn't return false)
		...
		if (this.currentsection!=i && i<this.sections.count && doload){ //if next section to show isn't the same as the current section shown
			...
			if (this.setting.revealfx[0]=="slide"){
				this.sections.$sections.css("display", "block")
				...
				this.sections.$innerwrapper.stop().animate({left:-i*this.maxfieldsetwidth}, this.setting.revealfx[1], function(){ //slide next section into view
					thiswizard.sections.$sections.each(function(thissec){
						if (thissec!=i) //hide fieldset sections currently not in veiw, so tabbing doesn't go to elements within them (and mess up layout)
							thiswizard.sections.$sections.eq(thissec).css("display", "none")
					})
				})
			}
			...
			this.paginatediv.$status.text("Langkah "+(i+1)+" dari "+this.sections.count) //update current page status text
			this.paginatediv.$navlinks.css('display', 'block')
			if (i==0) //hide "prev" link
				this.paginatediv.$navlinks.eq(0).css('display', 'none')
			else if (i==this.sections.count-1) //hide "next" link
				this.paginatediv.$navlinks.eq(1).css('display', 'none')
	...
	validate:function(section){
		...
		var validated=true, invalidtext=["Silakan isi field di bawah terlebih dahulu:\n"]
	...
	init:function(setting){
		...
		jQuery(function($){ //on document.ready
			...
			var $paginatediv=$('<div class="formpaginate" style="overflow:hidden;"><span class="prev" style="float:left">Kembali</span> <span class="status">Langkah 1 dari </span> <span class="next" style="float:right">Selanjutnya</span></div>')
			// var $paginatediv=$('<div class="formpaginate" style="overflow:hidden;"><span class="prev" style="float:left">Kembali</span> <span class="status">Langkah 1 dari </span> </div>')
